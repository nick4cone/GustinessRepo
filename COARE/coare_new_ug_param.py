...
def coare_new_ug_param(u, zu , t, zt, rh, zq, P, ts, sw_dn, lw_dn, lat, lon,jd, zi,rain, Ss, cp=None, sigH=None, zrf_u=10.0, zrf_t=10.0, zrf_q=10.0):   
#**************************************************************************
# VERSION INFO:
...
# input us = 0*u here;
    ...
    for i in np.arange(1,nits+1).reshape(-1):
        ...
        if np.size(zi) == 1:
            gust[k] = Beta * (np.multiply(Bf[k],zi)) ** 0.365
            del k
        else:
            gust[k] = Beta * (np.multiply(Bf[k],zi[k])) ** 0.365
            del k
        ut = np.sqrt(du ** 2 + gust ** 2)
        gf = ut / du
...
def albedo_vector(sw_dn = None,jd = None,lon = None,lat = None,eorw = None): 
    ...
    T[np.isnan(T)]=2  #to mimic matlab behavior on min function with NaNs
    ...
    if T.size==1:   ### for single value function
        ...
        for k in np.arange(0,np.size(sinpsi)).reshape(-1):
            Tchk = np.abs(Ts - T[k])
            i = np.array(np.where(Tchk == Tchk.min()))
            if psi[k] < 0: # or np.isnan(psi[k]) or i.shape[1]==0:
                alb[k] = 0
                solarmax[k] = 0
                T[k] = 0
                j = 0
...
# This code executes if 'run coare_new_ug_param.py' is executed from iPython cmd line
...
if __name__ == '__main__':
    ...
    A=coare_new_ug_param(u, zu , t, zt, rh, zq, P, ts, sw_dn, lw_dn, lat, lon,jd, zi,rain, Ss, cp , sigH, zrf_u, zrf_t, zrf_q)
    fnameA = os.path.join(path,'test_36_output_py_082022_withwavesinput.txt')
    # A=coare_new_ug_param(u, zu , t, zt, rh, zq, P, ts, sw_dn, lw_dn, lat, lon,jd, zi,rain, Ss, None , None, zrf_u, zrf_t, zrf_q)
   ...
   # A=coare_new_ug_param(u[0], zu[0], t[0], zt[0], rh[0], zq[0], P[0], ts[0], sw_dn[0], lw_dn[0], lat[0], lon[0],jd[0], zi[0],rain[0], Ss[0], cp[0] , sigH[0], zrf_u, zrf_t, zrf_q)
